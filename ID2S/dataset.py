...
def data_splitting(train_ratio, valid_ratio, seed, radius=5, splitting="all"):
    ...
    if splitting == "compound":
        num_host = np.max(data["host_index"]) + 1
        ...
        for i in range(len(data)):
            ...
            if data["host_index"][i] in train_idx:
                train_list.append(k)
            elif data["host_index"][i] in val_idx:
                valid_list.append(k)
            else:
                test_list.append(k)
...
def load_dataset(train_ratio, valid_ratio, seed, batch_size, splitting, radius):  # datset1,dataset2,dataset3
