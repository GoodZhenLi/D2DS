...
if __name__ == '__main__':
    ...
    start = time.time()
    lr = 0.0004
    result_path = "result_pretrain"
    checkpoint_path = 'checkpoint_pretrain'
    dataset_path = 'data'
    batch_size = 16
    cutoff=5
    for seed in seeds:
        checkpoint_name = f"{seed}_checkpoint.pt"
        ...
        model.to(device)
        run = Trainer(checkpoint_path=checkpoint_path, result_path=result_path,  train_ratio=0.9,
                      ...
                      lr_decay_factor=0.8, device=device, checkpoint_name=checkpoint_name,splitting="all",seed=seed,cutoff=cutoff)
        print(f"Start the {seed} model training")
