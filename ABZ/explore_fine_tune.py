...
if __name__ == '__main__':
    ...
    frozen_blocks=["atom_emb","atom_emb+interaction","interaction"]
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    for frozen_block in frozen_blocks:
        for splitting in splitting_ways:
            for train_ratio in train_ratio_list:
                for seed in seeds:
                    checkpoint_name = f"{frozen_block}_{train_ratio}_{splitting}_{seed}_checkpoint.pt"
                    result_file = f"{frozen_block}_{train_ratio}_{splitting}_{seed}_result.pkl"
                    ...
                    if frozen_block == "atom_emb+interaction":
                        for name, param in model.named_parameters():
                            if "emb" in name or "interaction" in name:
                                param.requires_grad = False

                    if frozen_block == "atom_emb":
                        for name, param in model.named_parameters():
                            if "emb" in name:
                                param.requires_grad = False
                    if frozen_block == "interaction":
                        for name, param in model.named_parameters():
                            if "interaction" in name:
                                param.requires_grad = False
                    x1 = sum(p.numel() for p in model.parameters() if not p.requires_grad)
                    x2 = sum(p.numel() for p in model.parameters())
                    print(f"frozen params: {x1 / x2}")
                    print(f"Start {frozen_block}-{train_ratio}-{splitting}-{seed} task")
                    model.to(device)
